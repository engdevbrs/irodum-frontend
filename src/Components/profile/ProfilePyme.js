...
const ProfilePyme = () => {
                ...
                if(result.isConfirmed){
                    ...
                    .then((result) => {
                        if(result.status === 200){
                            ...
                            Axios.get("http://ec2-54-174-104-208.compute-1.amazonaws.com:3001/api/download/speciality/" + dataUser[0].id)
                                .then((result) => {
                                    if(result.status === 200){
    ...
    const getAccess = (token) =>{
        Axios.post("http://ec2-54-174-104-208.compute-1.amazonaws.com:3001/api/user-info-pyme", {
            'authorization' : `${token}`
        }).then((result) => {
              if(result.status === 200){
