...
const Profile = () => {
                ...
                if(result.isConfirmed){
                    ...
                    .then((result) => {
                        if(result.status === 200){
                            ...
                            Axios.get("http://http://ec2-54-174-104-208.compute-1.amazonaws.com:3001/api/download/speciality/" + dataUser[0].idEmployed)
                                .then((result) => {
                                    if(result.status === 200){
    ...
    const getAccess = (token) =>{
        Axios.post("http://http://ec2-54-174-104-208.compute-1.amazonaws.com:3001/api/user-info", {
            'authorization' : `${token}`
        })
          .then((result) => {
              if(result.status === 200){
